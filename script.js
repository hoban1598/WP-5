      ...
      $('#backToSetup').on('click', function () {
        ...
        function draw() {
          ...
          if (!isQuizOpen) {
            ...
            if (areAllBricksCleared()) {
              console.log("✅ areAllBricksCleared() === true, 조건 만족됨");
              ...
              console.log("🟡 아직 벽돌 남아있음:", bricks.flat().filter(b => b && b.status === 1).length);
            }
          }
        ...
        function collisionDetection() {
            ...
            for (let r = 0; r < brickRowCount; r++) {
              ...
              if (b && b.status === 1) {
                if (
                  ...
                  // ✅ 이 부분만 유지 (setTimeout 제거)
                  if (areAllBricksCleared()) {
                    showItemModal(currentStage);

                    if (currentStage < 3) {
                      // 다음 스테이지로 넘어가는 로직이 있다면 여기에
                    } else {
                      // Stage 3인 경우, "계속하기" 눌렀을 때 엔딩화면으로 이동
                      $('#continueBtn').off('click').on('click', function () {
                        $('#itemModal').hide();
                        $('#game-container').hide();
                        $('#game-info').hide();
          ...
          function resetStage() {
            ...
            console.log("▶ bricks 배열:", bricks);
            console.log("▶ 전체 벽돌 수:", bricks.flat().length);
            console.log("▶ 남은 벽돌 수 (status===1):", bricks.flat().filter(b => b && b.status === 1).length);
          }
